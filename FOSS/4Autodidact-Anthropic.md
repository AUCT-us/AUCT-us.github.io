...
1. **Information Retrieval Foundations**
   ...
   - Explore [Retrival-Augmented Generation (RAG)](https://medium.com/@alexrodriguesj/understanding-retrieval-augmented-generation-rag-concepts-and-applications-0c5941bbdedc). RAG combines the strengths of two types of models: **retrieval models** which techniques like dense retrieval or sparse retrieveal, to fetch relevant documents or pieces of information from a large corpus based on a given query AND **generative models** which are neural networks, generally based on a tranformer model, eg GPT, BART, which is trained to leverage the context provided by the retrieved documents to generate more informativem coherent and contextually appropriate responses. Both, the retriever and generator are pre-trained on large-scale datasets separately, for optimizing their different purposes and then fine-tuned for greater degree of specific optimization on a task-specific dataset.
   - RAG applications help us understand why/how RAG is being used: 1) customer chat / sales, 2) talent acq / job hunt, 3) research assistance, 4) educational tools/context, 5) patent/legal research, 6) investment analysis, 7) product rec/comparison, 8) health/diet/fitness/supplment context, 9) travel maps/guides, 10) content generation
   - Naturally, we will want to examine the latest preprint papers in [vector search algorithms for information retrieval](https://arxiv.org/search/?query=%22vector+search+algorithms%22+information+retrieval&searchtype=all&abstracts=show&order=-announced_date_first&size=50) and [vector retrieval mathematical foundations](https://arxiv.org/abs/2401.09350) and generally explore as well as add to / annotate our [lists of theoretical papers](https://auctu.github.io/FOSS/7/)
   - Compare traditional IR techniques vs. [neural retrieval](https://arxiv.org/search/?query=%22neural+retrieval%22+information+retrieval&searchtype=all&source=header) approaches 
